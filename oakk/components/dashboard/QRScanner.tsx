...
export default function QRScanner({ onScanStart, onError }: QRScannerProps) {
  ...
  const stopScanner = useCallback(async () => {
    ...
    if (cameraActiveRef.current) {
      try {
        ...
        // Camera may already be stopping during component cleanup
      }
    ...
    try {
      ...
      // Scanner may already be cleared during component cleanup
    }
  }, []);
  ...
  const processCode = useCallback(async (decodedText: string) => {
    ...
    try {
      const result = await verifyAndCheckIn(decodedText);
      ...
      // Invalid or rejected check-in — show error inline
      setState("error");
  ...
  const startScanner = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        onError?.(msg || "Failed to start camera");
      }
    }
  }, [processCode, onScanStart, stopScanner, onError]);

  const scanFromGallery = useCallback(
    async (file: File) => {
      setState("reading");
      setErrorMsg("");
    ...
    [processCode, stopScanner, onError]
  );
